<script>
    ...
    const breadcrumbs = [
        ...
        {
            title: 'Create',
            url: route('admin.production-lines.create'),
            active: true
        }
    ];
    
    const pageTitle = 'Create Production Line';
    ...
    async function handleSubmit() {
        ...
        try {
            router.post(route('admin.production-lines.store'), formData, {
                onError: (err) => {
                    errors = err;
                    loading = false;
                },
        ...
        <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-2xl font-bold text-mono">Create Production Line</h1>
                    <p class="text-sm text-secondary-foreground">
                        Define a new production line with outputs and manufacturing steps
                    </p>
                </div>
